...
|---|---|---|---|
| D1 | Captioner | **DECIDED 2026-06-10: `QuantTrio/Qwen3.5-9B-AWQ` (tag `a`) wins the bake-off** — parse 99.5% vs 95.3% (B: 71 JSONDecodeErrors), event recall 26.6% vs 1.4% (clips) and 39.7% vs 0.7% (event windows), throughput 4094 vs ~3500 req/h on 4 workers. Qwen3-VL-8B-FP8 (tag `b`) captions retained as a robustness comparison set only. (History: researched ID `Qwen/Qwen3.5-9B-FP8` does not exist — caught in Phase 0 ID verification.) | Phase 2 bake-off, artifacts/captions/ |
